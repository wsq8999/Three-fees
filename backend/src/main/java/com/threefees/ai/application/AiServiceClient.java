...
public class AiServiceClient {
  ...
  static String reportStyleGuide(String normalizedIntent) {
    ...
    return """
        ...
        二、排查分析优先写当前业务原因、设备情况、分摊比例变化、电信下电、设备新增、空调运行、额定功率测算等；可写“本期电量同比超标原因：……”这类正式报告句式。
        ...
        三、整改小结不超过 120 字，写成 1 个短段，优先落到已完成核查、不存在跑冒滴漏、不存在偷搭电、实际用电情况正常；只有当前材料明确支持时，才写台账已更新或分摊比例已更新。
        禁止把“需复核更新台账”“需复核并更新额定功率台账”“现场设备台账与系统额定功率标杆存在偏差”作为默认结论；证据不足时可写“相关原因待现场材料进一步核实”，不得强行落到台账复核。
        ...
        输出 updatedSections 前必须校对中文错别字、业务名词和 HTML 实体残留；“额亏”必须改为“额定”，“阙值”必须改为“阈值”或当前口径“正常上限”，“集稽核”必须改为“稽核”。
        updatedSections 中不得输出“&#x20;”等 HTML 实体残留，不得保留异常多余空格；设备型号、运营商名称和数字单位不得误改。
